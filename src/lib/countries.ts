/**
 * Complete list of world countries with ISO codes and nationality demonyms.
 * Single source of truth for all dropdowns (nationality, document country, etc.)
 */
...
/** Sorted nationality list for dropdowns (Bangladesh first) */
export const NATIONALITY_OPTIONS = (() => {
  const bd = COUNTRIES.find(c => c.code === "BD")!;
  const rest = COUNTRIES.filter(c => c.code !== "BD").sort((a, b) => a.nationality.localeCompare(b.nationality));
  return [bd, ...rest];
})();

/** Sorted country list for dropdowns (Bangladesh first) */
export const COUNTRY_OPTIONS = (() => {
  const bd = COUNTRIES.find(c => c.code === "BD")!;
  const rest = COUNTRIES.filter(c => c.code !== "BD").sort((a, b) => a.name.localeCompare(b.name));
  return [bd, ...rest];
})();
