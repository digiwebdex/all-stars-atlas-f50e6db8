...
const AdminDiscounts = () => {
  ...
  const filteredRules = mockPriceRules.filter(r => {
    if (search && !r.name.toLowerCase().includes(search.toLowerCase())) return false;
    if (serviceFilter !== "all" && r.service !== serviceFilter) return false;
    return true;
  });

  const totalActive = mockDiscounts.filter(d => d.status === "active").length;
  const totalRedemptions = mockDiscounts.reduce((sum, d) => sum + d.usageCount, 0);
  const avgDiscount = Math.round(mockDiscounts.filter(d => d.type === "percentage").reduce((sum, d) => sum + d.value, 0) / mockDiscounts.filter(d => d.type === "percentage").length);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Price Rules", value: mockPriceRules.filter(r => r.status === "active").length, icon: DollarSign, color: "text-accent" },
        ].map((s, i) => (
          <Card key={i}>
      ...
      <Tabs defaultValue="discounts" className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-3 flex-1 w-full">
              <div className="relative flex-1 max-w-md">
                ...
                <Input placeholder="Search discounts or codes..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
              <Select value={serviceFilter} onValueChange={setServiceFilter}>
                <SelectTrigger className="w-full sm:w-[150px]"><SelectValue placeholder="Service" /></SelectTrigger>
                <SelectContent>
                  ...
                  <SelectItem value="All Services">Universal</SelectItem>
                </SelectContent>
              </Select>
            ...
            <Dialog open={showCreateDiscount} onOpenChange={setShowCreateDiscount}>
              <DialogTrigger asChild>
                <Button><Plus className="w-4 h-4 mr-1.5" /> New Discount</Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg">
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <Input placeholder="e.g. Summer Flight Sale" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label>Code</Label>
                      <Input placeholder="e.g. SUMMER30" className="uppercase font-mono" />
                    </div>
                    <div className="grid gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="activate-now" className="text-sm">Activate immediately</Label>
                  </div>
                </div>
                <DialogFooter>
                  <DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose>
                  <Button onClick={() => { setShowCreateDiscount(false); toast({ title: "Discount Created", description: "New discount code has been created successfully." }); }}>
                    Create Discount
                  </Button>
                </DialogFooter>
              </DialogContent>
                    ...
                    <TableRow key={d.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          <code className="text-xs bg-muted px-2 py-0.5 rounded font-bold">{d.code}</code>
                          <button onClick={() => copyCode(d.code)} className="text-muted-foreground hover:text-foreground transition-colors">
                      ...
                      <TableCell className="hidden md:table-cell">
                        <Badge variant="outline" className="text-[10px]">{d.service}</Badge>
                      </TableCell>
                      <TableCell className="hidden lg:table-cell text-sm">
                        {d.usageCount}/{d.usageLimit}
                      </TableCell>
                      <TableCell className="hidden lg:table-cell text-xs text-muted-foreground">
                        ৳{d.minOrder.toLocaleString()} / ৳{d.maxDiscount.toLocaleString()}
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={`text-[10px] capitalize ${statusColors[d.status] || ""}`}>
                          {d.status}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => toast({ title: "View Discount", description: `Viewing details for ${d.name}` })}>
                              <Eye className="w-4 h-4 mr-2" /> View Details
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => toast({ title: "Edit Discount", description: `Editing ${d.name}` })}>
                              <Edit2 className="w-4 h-4 mr-2" /> Edit
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => copyCode(d.code)}>
                              <Copy className="w-4 h-4 mr-2" /> Copy Code
                            </DropdownMenuItem>
                            <DropdownMenuItem className="text-destructive" onClick={() => toast({ title: "Discount Deleted", description: `"${d.code}" has been removed.`, variant: "destructive" })}>
        ...
        <TabsContent value="pricing" className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <div className="relative flex-1 max-w-md">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder="Search price rules..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>

            {/* Create Price Rule Dialog */}
            <Dialog open={showCreateRule} onOpenChange={setShowCreateRule}>
              <DialogTrigger asChild>
                <Button><Plus className="w-4 h-4 mr-1.5" /> New Price Rule</Button>
              </DialogTrigger>
              <DialogContent>
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <Input placeholder="e.g. Flight Base Markup" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label>Service</Label>
                      <Select defaultValue="Flights">
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="Flights">Flights</SelectItem>
                          <SelectItem value="Hotels">Hotels</SelectItem>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select defaultValue="markup">
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="convenience">Convenience Fee</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input placeholder="e.g. All flights, Domestic only" />
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <DialogFooter>
                  <DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose>
                  <Button onClick={() => { setShowCreateRule(false); toast({ title: "Price Rule Created", description: "New pricing rule has been saved." }); }}>
                    Save Rule
                  </Button>
                </DialogFooter>
              </DialogContent>
                ...
                <TableBody>
                  {filteredRules.length === 0 ? (
                    ...
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={`text-[10px] capitalize ${statusColors[r.status] || ""}`}>
                          {r.status}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => toast({ title: "Edit Rule", description: `Editing ${r.name}` })}>
                              <Edit2 className="w-4 h-4 mr-2" /> Edit
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => toast({ title: r.status === "active" ? "Rule Paused" : "Rule Activated", description: `${r.name} has been ${r.status === "active" ? "paused" : "activated"}.` })}>
                              <TrendingDown className="w-4 h-4 mr-2" /> {r.status === "active" ? "Pause" : "Activate"}
                            </DropdownMenuItem>
                            <DropdownMenuItem className="text-destructive" onClick={() => toast({ title: "Rule Deleted", description: `"${r.name}" has been removed.`, variant: "destructive" })}>
                              <Trash2 className="w-4 h-4 mr-2" /> Delete
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
